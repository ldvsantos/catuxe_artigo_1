...
\title{\textbf{Prontidão do Aprendizado de Máquina para Suporte à Transição Agroecológica em Sistemas Tradicionais. Uma meta-análise}}
...
Quanto as limitações podemos observar que, o intercepto de Egger ($\beta_0 = 1{,}87$; $p = 0{,}009$) e a correção \textit{trim-and-fill} ($-$1,54 p.p.) classificam a acurácia combinada de 90,66\% como limite superior do desempenho \textit{reportado}, não como benchmark operacional; a análise de sensibilidade excluindo os 37,2\% dos estudos com n imputado confirmou estabilidade da estimativa (89,8\% vs. 90,7\%), indicando que a imputação não introduziu desvio material. A ausência de instrumentos formais de avaliação de viés em nível de estudo (PROBAST, QUADAS-2) impede estratificação por qualidade metodológica, enquanto a restrição a termos em inglês nas bases Scopus e Web of Science pode sub-representar iniciativas de monitoramento em regiões não anglofônicas. Por fim, estratégias de validação espacial não foram sistematicamente extraídas, limitando a quantificação empírica da degradação de desempenho em validade externa. Em conjunto, os achados proveem um referencial diagnóstico do estado atual, não benchmarks prontos para implantação regulatória.
