...
\includegraphics[width=0.65\textwidth]{../2-FIGURAS/2-EN/louvain_modules_detailed.png}%
}{%
...
\centering
\includegraphics[width=0.65\textwidth]{../2-FIGURAS/2-EN/fair_radar_only.png}
\par\small (a)
\end{minipage}
\hfill
\begin{minipage}[t]{0.49\textwidth}
...
No eixo de desempenho, médias elevadas na síntese meta-analítica (Figura \ref{fig:meta_algoritmo}) exigem leitura orientada por heterogeneidade e sensibilidade do reporte, pois a estimativa combinada em torno de 90,66\% coexiste com dispersão entre estudos refletida por $I\textsuperscript{2}=58,00\%$ e por um $Q$ altamente significativo, de modo que acurácia agregada não constitui evidência suficiente de estabilidade fora de domínio quando faltam descritores reprodutíveis de treinamento, particionamento espacial e validação externa. 

A assimetria estatística compatível com viés de publicação, detectada por regressão de Egger (estatística 40,02, p=$7,31\times 10^{-74}$) e por correlação de postos (estatística 0,991, p=$1,71\times 10^{-56}$), além da imputação de 42 estudos pelo \textit{trim-and-fill}, reforça que parte do desempenho reportado pode refletir seleção e subnotificação de resultados menos favoráveis, deslocando a discussão do valor médio para a verificabilidade do pipeline. Nesse enquadramento, a tendência temporal positiva na meta-regressão, ainda que marginal (p=0,0866), é mais compatível com intensificação de práticas de modelagem e disponibilidade de dados do que com ganho garantido de generalização, o que amplia a necessidade de transparência e justificabilidade em sistemas de decisão automatizada \citep{Rudin2019} e torna vulnerável qualquer regime de certificação ancorado apenas em métricas opacas.
