...
Nesse contexto, o Aprendizado de Máquina (ML) emerge como ferramenta promissora para operacionalizar, no sentido de um gêmeo digital inferencial orientado à evidência e à validação, uma representação computacional do sistema socioecológico acoplada a observações, continuamente atualizada por dados e orientada à simulação, monitoramento e suporte à decisão \citep{Jones2020DigitalTwin,VanDerHorn2021DigitalTwin,Mountrakis2011,Spyrou2025}.
...
A transparência causal beneficia-se de métodos de Inteligência Artificial Explicável (XAI) que permitam discriminar marcadores físico-químicos de correlações espúrias \citep{Rudin2019}. Além disso, a soberania de dados é favorecida pela adesão aos princípios FAIR e por trilhas de auditoria imutáveis, sustentando reprodutibilidade e rastreabilidade da inferência \citep{Wilkinson2016,Kshetri2014}.
...
No pré-processamento espectral, aplicou-se Análise de Componentes Principais (PCA) sobre matriz de reflectância normalizada por pixel, utilizando a matriz de correlações de Pearson como base de decomposição. Adotou-se rotação varimax e mantiveram-se os componentes cujo autovalor superou a fronteira de Kaiser ($\lambda>1$), sendo este limiar corroborado pelo cotovelo da curva scree. Tal procedimento resultou em cinco componentes principais que, em conjunto, concentraram 92,4\% da variância acumulada — patamar que assegura que o espaço projetado retém a estrutura informacional essencial do cubo de dados. A estabilidade da solução foi confirmada por re-amostragem bootstrap (1\,000 iterações), cuja dispersão interquartil dos autovalores manteve-se inferior a 3,1\%, validando a robustez do subespaço gerado.
