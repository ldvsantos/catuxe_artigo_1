...
\title{\textbf{Auditoria Socioecológica Digital em Sistemas Agrícolas Tradicionais, revisão de escopo e estado da arte em aprendizado de máquina}}
...
\abstract{Sistemas Agrícolas Tradicionais (SAT) são sistemas socioecológicos acoplados, nos quais manejo, diversidade biocultural e condições biofísicas coevoluem, e a governança depende de evidências verificáveis em escala territorial. Esta revisão de escopo mapeia como o Aprendizado de Máquina tem sido aplicado a SAT e em que medida desenhos de validação e governança de dados sustentam auditoria socioecológica de serviços ecossistêmicos e integridade. Seguindo PRISMA-ScR, a busca em Scopus e Web of Science recuperou 449 registros, e 244 estudos publicados entre 2010--2025 foram incluídos após triagem automatizada por pontuação ponderada (precisão 94,2\%) e verificação manual (CCI=0,87). A síntese combinou análise multivariada, redes de coocorrência e agrupamento de perfis, e avaliou governança segundo FAIR. Observou-se uma transição para Deep Learning e sensoriamento remoto, mas com heterogeneidade de objetivos e contextos que limita comparação direta entre SAT e, portanto, a transferibilidade de indicadores. A robustez preditiva global foi condicionada pela estrutura relacional do corpus, que exibiu modularidade $Q=0{.}183$, indicando especializações temáticas que reduzem interoperabilidade entre abordagens quando metadados e protocolos não são padronizados. A aderência a FAIR foi baixa, com score total de 18{.}667/100, restringindo reprodutibilidade e contraprova independente, condição crítica para auditorias e tomada de decisão baseada em evidência em paisagens agrícolas tradicionais. No subconjunto com dados comparáveis ($k=16$), a acurácia combinada foi maior em Deep Learning (92{.}872\%) do que em Random Forest (85{.}700\%) e SVM (85{.}000\%), mas a evidência converge para o mesmo requisito operacional, a auditabilidade em SAT depende sobretudo de validação verificável, transparência analítica e infraestrutura de dados reusável.}
...
A legitimidade contemporânea de estratégias de salvaguarda e valorização associadas a SAT, quando vinculada a reconhecimento patrimonial, políticas de conservação e instrumentos de rastreabilidade, tende a emergir de arranjos institucionais e comunitários que reduzem assimetrias informacionais, sobretudo quando alegações de sustentabilidade são mobilizadas para acesso a mercados, recursos ou programas públicos \citep{Gale2023}. A credibilidade desses arranjos tende a depender de demonstração empírica de que a continuidade do sistema se ancora em interações socioecológicas específicas, com dependências territoriais nem sempre replicáveis fora do contexto local, e de que variações no manejo e no regime climático não degradam a integridade do conjunto de funções ecossistêmicas e culturais que estrutura o sistema.
...
Embora a complexidade sistêmica e o caráter difuso dos acoplamentos biofísicos e sociais possam limitar a detecção de serviços ecossistêmicos por métricas convencionais, a governança de bens comuns depende de evidências verificáveis que reduzam espaço para práticas de \textit{greenwashing} quando a retórica de sustentabilidade não é acompanhada por contraprova independente \citep{Levin1998ComplexAdaptiveSystems,Gale2023}. Dada a insuficiência do instrumental analítico clássico para monitoramento em escala de paisagem \citep{Liao2023}, a construção de evidências auditáveis tende a exigir arquiteturas computacionais que conectem observações multiescalares a critérios explícitos de validação.
...
Nesse enquadramento, este estudo tem como objetivo mapear como o Aprendizado de Máquina tem sido aplicado a Sistemas Agrícolas Tradicionais (SAT) e em que medida desenhos de validação e governança de dados sustentam auditoria socioecológica de serviços ecossistêmicos e integridade. Considera-se que um gêmeo digital inferencial orientado a SAT, fundamentado na reconstrução computacional dinâmica de interações entre o genótipo territorial, entendido como condições biofísicas e ecológicas, e o fenótipo socioecológico, entendido como práticas, outputs e assinaturas observáveis do sistema, quando sustentado por validação espaço-temporal rigorosa e mecanismos de segurança e governança de dados, tende a produzir evidências auditáveis compatíveis com decisões de salvaguarda e gestão territorial.

\section[Materiais e Métodos]{Materiais e Métodos}

\subsection[Fluxo de Seleção]{Fluxo de Seleção Bibliográfica (PRISMA)}

O delineamento foi conduzido como revisão de escopo e seguiu as diretrizes PRISMA-ScR \citep{tricco2018,MUNN2018}. A questão de pesquisa foi formulada para mapear como o Aprendizado de Máquina tem sido aplicado a Sistemas Agrícolas Tradicionais e em que medida desenhos de validação e governança de dados sustentam auditoria socioecológica de serviços ecossistêmicos e integridade, o que orientou a estratégia de busca, a triagem e a extração de variáveis metodológicas do corpus. 

As fontes de informação foram Scopus e Web of Science, com buscas avançadas em campos equivalentes a título, resumo e palavras-chave, empregando uma estratégia booleana que combinou termos de domínio associados a SAT, incluindo \textit{traditional agricultural system}, \textit{traditional farming system}, \textit{traditional agroecosystem}, \textit{sistemas agrícolas tradicionais}, \textit{indigenous farming}, \textit{shifting cultivation} e \textit{slash-and-burn}, com termos de método associados a aprendizado de máquina e sensoriamento remoto, incluindo \textit{machine learning}, \textit{artificial intelligence}, \textit{deep learning}, \textit{random forest}, \textit{support vector machine}, \textit{neural network}, \textit{CNN}, \textit{LSTM}, \textit{yolo} e \textit{remote sensing}. O recorte temporal foi definido como 2010 a 2025, compatível com a emergência e consolidação de aplicações de ML em observação da Terra e em monitoramento ambiental aplicado a sistemas agrícolas tradicionais.

\begingroup
\footnotesize
\ttfamily
\noindent A string utilizada na Scopus foi\par
...
\noindent A string utilizada na Web of Science foi\par
\noindent TS=(("traditional agricultural system*" OR "traditional farming system*" OR "traditional agriculture" OR "traditional agroecosystem*" OR "sistemas agrícolas tradicionais" OR "sistema agrícola tradicional" OR "agricultura tradicional" OR "indigenous farming" OR "indigenous agriculture" OR "shifting cultivation" OR "slash-and-burn" OR "ancestral farming" OR "ancient agriculture") AND ("machine learning" OR "artificial intelligence" OR "deep learning" OR "random forest" OR "neural network*" OR "support vector machine*" OR "SVM" OR "decision tree*" OR "gradient boosting" OR "CNN" OR "long short-term memory" OR "LSTM" OR "yolo" OR "remote sensing")) AND PY=(2010-2025)\par
\endgroup

Os registros exportados foram consolidados e processados em um único corpus para reduzir duplicidades entre bases e padronizar metadados antes da triagem, com o fluxo documentado no diagrama PRISMA-ScR (Fig. \ref{fig:prisma}). A busca inicial recuperou 449 registros combinados das bases Scopus e Web of Science. Após triagem automatizada por pontuação ponderada e verificação manual de elegibilidade, 244 estudos foram incluídos na síntese qualitativa e quantitativa.
...
As estratégias analíticas foram articuladas para responder a uma questão de coerência operacional, na qual a maturidade técnica do campo só se torna relevante para SAT quando pode ser conectada a requisitos mínimos de auditabilidade. Nesse sentido, a caracterização temporal e relacional delimita como o campo se organiza e se fragmenta, as sínteses multivariadas e de agrupamento descrevem a heterogeneidade de perfis metodológicos e contextuais, a meta-análise e a meta-regressão testam a estabilidade da evidência quantitativa reportada e sua sensibilidade a regimes de validação, e a avaliação FAIR fornece um diagnóstico de governança que condiciona reprodutibilidade e contraprova independente \citep{Wilkinson2021}.

Para assegurar reprodutibilidade e auditabilidade, a busca e a triagem foram executadas por scripts disponibilizados no repositório local do projeto, com consolidação de metadados, deduplicação entre bases e registro do fluxo PRISMA \citep{tricco2018,MUNN2018}. A elegibilidade exigiu aplicação de aprendizado de máquina com dados empíricos em Sistemas Agrícolas Tradicionais ou contextos diretamente comparáveis, reportes explícitos de acurácia e tamanho amostral para inclusão na síntese quantitativa, e presença de regime de validação definido na forma de validação cruzada ou validação espacialmente independente quando aplicável \citep{Kuhn2013}. Estudos sem dados originais, revisões narrativas, relatos com opacidade analítica ou sem possibilidade de contraprova foram excluídos. 

A pontuação final de seleção foi obtida por função aditiva sobre componentes normalizados entre zero e um para qualidade metodológica, relevância temática e impacto bibliométrico, com pesos definidos a priori de modo a privilegiar verificabilidade e transparência. As estimativas estatísticas adotaram intervalos de confiança de noventa e cinco por cento, modelos de efeitos aleatórios quando a heterogeneidade justificou este enquadramento, e meta-regressão com moderadores operacionais, incluindo família algorítmica, fonte de dados e regime de validação. A estrutura de redes de coocorrência foi construída como grafo não direcionado e ponderado e analisada com \texttt{networkx}, com filtragem mínima de coocorrências para reduzir ruído e tornar comparáveis as estatísticas estruturais reportadas \citep{Schoch2020}.
...
\end{minipage}
\caption{Rede de coocorrência de valores categóricos no corpus de SAT, na qual (a) apresenta a topologia completa e (b) destaca a decomposição em comunidades.}
\par{\footnotesize\textit{Nota.} Na visão completa, as cores identificam a dimensão de origem de cada nó, permitindo rastrear a conectividade entre Algoritmo, Aplicação, Contexto, Evidência e Região. Na visão por comunidades, as cores indicam grupos de valores categóricos que coocorrem com maior frequência entre si do que com o restante da rede. O tamanho dos nós representa o grau, isto é, o número de conexões de cada nó. A espessura das arestas representa o peso, isto é, a frequência de coocorrência entre pares de nós.}\par
...
\par{\footnotesize\textit{Nota.} Na sub-rede algoritmo\texttimes{}aplicação, as cores distinguem as duas dimensões, evidenciando combinações recorrentes entre famílias algorítmicas e tarefas de aplicação. Na síntese por métricas, cada painel corresponde a uma medida de centralidade calculada no maior componente conectado, permitindo identificar nós com maior conectividade local e maior potencial de intermediação no grafo de coocorrência.}\par
...
\subsection[Análise de clusters e perfis]{Análise de clusters e perfis metodológicos}
...
\caption{Heatmap do perfil médio de atributos categóricos por cluster no subconjunto com categorização completa utilizado para agrupamento (2010--2025).}
\par{\footnotesize\textit{Nota.} As linhas representam atributos no formato Dimensão=Categoria e as colunas representam os clusters, cujos rótulos no eixo sintetizam categorias dominantes e o tamanho amostral. O dendrograma à esquerda ordena os atributos por similaridade no espaço de perfis médios, com ligação hierárquica pelo método de Ward, e os rótulos inseridos nos ramos identificam apenas agrupamentos internos com mais de três folhas, sinalizando de forma compacta a dominância relativa entre clusters e as dimensões mais recorrentes naquele subgrupo. A intensidade de cor expressa a presença média por cluster no intervalo 0--100\%.}\par
...
O conjunto de dados que sustenta os resultados, incluindo corpus bibliográfico consolidado, variáveis categorizadas, strings de busca e scripts de análise, será depositado em Zenodo sob acesso aberto, com DOI a ser informado na versão final após a publicação. Enquanto o DOI não é atribuído, os arquivos estão organizados para depósito em \texttt{8-REVISÃO\_ESCOPO\_SAT/zenodo\_release}, com manifesto e metadados prontos para submissão.
